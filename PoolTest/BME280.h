...
class BME280
{
 	public:
		...
		//Return vector <temp, humidity, pressure>
		std::vector<double> readAll(void);
		...
		//Writes a byte;
    		...
    		void readlog(void);
    		FILE * pATMOFile;
    		char atmofilename[100];
    		char atmofile[50];
		struct timeval tval_before_atmo;
		time_t nowtime;
        	struct tm *nowtm;
  	...
  	private:
		...
		int file;
		int adapter_nr; /* beaglebone i2c bus number */
		char filename[20];

...
MS5837::MS5837() {
  	...
  	if (ioctl(file, I2C_SLAVE, MS5837_ADDR) < 0) 
	{
  	...
  	if(pPRESFile == NULL)
  	{
	  	freopen(presfilename,"a",pPRESFile);
  	}

}

void MS5837::readlog(void)
{
	gettimeofday(&tval_before_pres, NULL);
	read_pressure();
	
	double pres, temp, depth;
	pres = getPressure();
	temp = getTemperature();
	depth = getDepth();

	char tmbuf[64];
	nowtime = tval_before_pres.tv_sec;
	...
	//printf("%s",buffer);
	fwrite(bufptr,sizeof(char),txo,pPRESFile);
	//fflush(pIMUFile);
...
float MS5837::altitude() {
	//return (1-pow((getPressure()/1019.90),.190284))*145366.45*.3048;

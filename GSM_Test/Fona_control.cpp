...
void Fona_control::sms_location_send(void)
{
	...
	char cmd1[] = "AT+CMGS=\"15097420901\"\r";
	char msg[100];
	int n = snprintf(msg,100,"Hi. The GPS Date/Time is: %i/%02i/%02i-%02i:%02i:%.3f",year,month,day,hour,minute,seconds);
	char ctrl_z = 0x1A;
...
void Fona_control::begin(void)
{
	...
	memset(buffer1,'\0',50);
/*
	n = write(fona_fd,PMTK_SET_BAUD_38400,sizeof(PMTK_SET_BAUD_38400)-1);
	printf("Writ: %i\n",n);
	n = read(fona_fd,buffer1,6);
	printf("%s\n",buffer1);
	memset(buffer1,'\0',50);
*/
